...
**S3 Versioning** — це механізм, який зберігає **кожну версію** кожного об'єкту у bucket. Якщо ви завантажили `logo.png`, потім знову завантажили `logo.png` (інший файл з тією ж назвою) — S3 зберігає обидві версії. Стара не перезаписується.

**Навіщо потрібне версіонування:**
- Захист від випадкового видалення: видалений об'єкт отримує «Delete Marker», і його можна відновити
- Захист від перезапису: можна повернутись до попередньої версії файлу
- Відповідність регуляторним вимогам (GDPR, compliance): зберігати всі версії документів
- Відкат після деплою: якщо новий build фронтенду зламав сайт — повернути попередній

**Стани Versioning:**
- `Suspended` (за замовчуванням): версіонування вимкнене. Нові об'єкти зберігаються без версій (Version ID = `null`).
- `Enabled`: увімкнено. Кожен об'єкт отримує унікальний Version ID. Зупинити після увімкнення — не можна. Можна лише `Suspend` (заблокувати нові версії).

```bash
# Увімкнути версіонування на bucket
aws s3api put-bucket-versioning \
    --bucket my-app-bucket \
    --versioning-configuration Status=Enabled \
    --region eu-central-1

# Переглянути всі версії об'єкту
aws s3api list-object-versions \
    --bucket my-app-bucket \
    --prefix "uploads/avatar.png"
# Відповідь містить Versions[] та DeleteMarkers[]

# Відновити попередню версію (замініть VERSION_ID)
aws s3api copy-object \
    --bucket my-app-bucket \
    --copy-source "my-app-bucket/uploads/avatar.png?versionId=abc123def456" \
    --key "uploads/avatar.png"
```

::caution
Версіонування суттєво збільшує витрати на зберігання: кожна версія кожного файлу займає місце і тарифікується. Обов'язково налаштуйте Lifecycle Policy для автоматичного видалення старих версій!
::
...
**Lifecycle Policy** — це набір правил, що автоматично переміщують або видаляють об'єкти через певний час. Це ключовий інструмент для оптимізації витрат та управління даними.

**Типові сценарії:**
- Логи зберігати у Standard 30 днів → перемістити в Standard-IA → через 90 днів видалити
- Резервні копії: Standard 7 днів → Glacier 30 днів → Deep Archive 365 днів → видалити через 7 років
- Старі версії (при Versioning): зберігати поточну → видаляти попередні через 30 днів

**Приклад: оптимізація логів:**
...
Читається: файли у `logs/` → через 30 днів перемістити у Standard-IA → через 90 днів у Glacier → через 365 днів видалити. Некурентні версії (при Versioning) — видаляти через 30 днів.
...
Безпека S3 — це один із найскладніших і найважливіших аспектів. Помилки в налаштуванні S3 bucket стали причиною гучних витоків даних (Capital One, GoDaddy, NASA та інші).

### Block Public Access — перший захист

**Block Public Access** — це налаштування на рівні bucket (і на рівні акаунту!), яке **перевизначає** всі інші дозволи та гарантує, що bucket залишається приватним, навіть якщо є Policy, що дозволяє публічний доступ.

```bash
# Заблокувати ВЕСЬ публічний доступ до bucket (рекомендовано за замовчуванням)
...
::caution
AWS рекомендує: **усі bucket за замовчуванням приватні**. Увімкнення публічного доступу — навмисна дія. Перед відкриттям публічного доступу переконайтесь, що це справді необхідно.
::

### Bucket Policy — JSON-правила доступу

**Bucket Policy** — це IAM-подібний JSON-документ, який визначає хто може що робити з bucket та його об'єктами. На відміну від IAM Policy (прикріпленої до User/Role), Bucket Policy прикріплена до самого bucket.

**Приклад: публічний доступ на читання для static website:**
...
**Приклад: доступ лише для конкретної Lambda функції:**
...
**Приклад: доступ лише з певного VPC (для private API):**
...
**SSE-S3 (Server-Side Encryption with S3 Keys):** AWS автоматично шифрує всі об'єкти при завантаженні та розшифровує при читанні. Ключами керує AWS. Безкоштовно. **Рекомендовано вмикати на всіх bucket за замовчуванням.**

**SSE-KMS (Server-Side Encryption with KMS Keys):** шифрування через AWS KMS. Ви контролюєте ключ, маєте аудитний лог кожного використання ключа, можете відкликати доступ. Підходить для чутливих даних (медична, фінансова інформація).

**SSE-C (Customer-Provided Keys):** ви самі надаєте ключ при кожному запиті. AWS не зберігає ключ. Складно управляти — рідко використовується.

```bash
# Встановити SSE-S3 за замовчуванням для bucket
