...
- .NET SDK встановлений локально
- VPC з public та private subnets (якщо немає — AWS надає Default VPC)
...
```bash
# Крок 1a: Знайти Default VPC та Subnets
...
# Крок 1b: Створити DB Subnet Group (потрібна для RDS)
...
# Крок 1c: Створити Security Group для RDS
...
# Дозволити з'єднання на порт 5432 тільки всередині VPC
...
### Крок 2: Налаштування Security Group для доступу з EC2

Після створення RDS — дозвольте з'єднання з Security Group вашого EC2 instance:

```bash
EC2_SG_ID="sg-0123456789abcdef0"  # SG вашого EC2 де живе .NET API
...
### Крок 4: Міграція та перевірка

```bash
# Додати connection string у appsettings.Development.json для локального dev
# (для prod — через змінну середовища)
...
```bash
# Перевірити що таблиці створено — через psql або будь-який PostgreSQL клієнт
# (підключення можливе лише зсередини VPC або через SSH-тунель)
psql "host=$RDS_ENDPOINT dbname=myapp user=postgres sslmode=require" -c "\dt"
```

### Крок 5 (ОБОВ'ЯЗКОВО): Очищення

::caution
RDS instance `db.t3.micro` поза Free Tier коштує ~$0.018/год ≈ $13/місяць. Free Tier дає 750 год/місяць протягом першого року. Після лабораторної — видаліть instance.
::

```bash
# Видалити RDS з фінальним snapshot
aws rds delete-db-instance \
    --db-instance-identifier my-dotnet-app-db \
    --final-db-snapshot-identifier my-dotnet-app-db-final \
    --region eu-central-1

# Або без snapshot (якщо дані не потрібні)
aws rds delete-db-instance \
    --db-instance-identifier my-dotnet-app-db \
    --skip-final-snapshot \
    --region eu-central-1

# Після видалення instance (статус deleted) — прибрати залежні ресурси
