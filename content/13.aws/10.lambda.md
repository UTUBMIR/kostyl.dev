...
description: Глибоке дослідження AWS Lambda — від фундаментальних принципів serverless архітектури до продакшн-оптимізації .NET функцій. Runtimes, Layers, Triggers, Cold Start, Provisioned Concurrency, Lambda Destinations та повна інтеграція з екосистемою AWS для .NET 10 розробників.
---
...
::card{title=".NET 10" icon="i-simple-icons-dotnet"}
...
_Завдяки цьому файлу вам не потрібно щоразу вводити довгі параметри в терміналі._
...
::card{title="Зменшення пакетів" icon="i-heroicons-archive-box"}
...
::card{title="Централізація" icon="i-heroicons-arrows-pointing-in"}
Змінюйте версію спільної бібліотеки в одному шарі, замість того щоб перезбирати та деплоїти 50 різних функцій.
::

::card{title="Розподіл праці" icon="i-heroicons-users"}
...
_Після цього видаліть зайві файли (.csproj, .json), залишивши лише потрібні .dll._
...
::card{title="Amazon.Lambda.Core" icon="i-heroicons-cpu-chip"}
...
::card{title="Amazon.Lambda.Serialization.SystemTextJson" icon="i-heroicons-arrows-right-left"}
...
**Механізм реєстрації серіалізатора:**
AWS Lambda не знає за замовчуванням, як десеріалізувати вхідні дані. Ви ПОВИННІ вказати серіалізатор на рівні assembly (найкраща практика) або конкретного методу:
...
::field{name="InvokedFunctionArn" type="string"}
...
::warning
**Особливість деплойменту:** Оскільки Lambda Annotations генерує конфігурацію у файл `serverless.template`, ви **не можете** деплоїти такі функції звичайною командою `dotnet lambda deploy-function`. Ви ПОВИННІ використовувати команду деплою всього стеку:
`dotnet lambda deploy-serverless --stack-name my-api-stack --s3-bucket my-bucket --region eu-central-1`
::
...
::note
**Важливий нюанс для .NET**: Оскільки функція очікує чистий JSON-рядок, у утиліті `curl` ми передаємо текст у подвійних лапках всередині одинарних (`'"hello"'`), щоб серіалізатор `SystemTextJson` правильно його розпізнав. 
Результатом у консолі буде: `"HELLO FROM INTERNET"`.
::
...
1. Перейдіть у сервіс **IAM** → **Roles** → **Create role**.
2. Trusted entity: **AWS service** → **Lambda**. Натисніть **Next**.
...
1. Перейдіть до **DynamoDB** → **Tables** → **Create table**.
...
::terminal-preview{title="Response 201 Created"}
...
::card{title="Академічний висновок" icon="i-heroicons-academic-cap"}
...
1. Перейдіть до **CloudFormation** → **Stacks**.
2. Оберіть `url-shortener-stack` та натисніть **Delete**.
3. Підтвердіть видалення — AWS автоматично видалить Lambda, API Gateway та IAM ролі, що були створені стеком.
4. Окремо перейдіть до **DynamoDB** → **Tables**, оберіть `UrlMappingTable` та натисніть **Delete table**.
...
::card{title="Stateless за замовчуванням" icon="i-heroicons-arrow-path"}
...
::card{title="Cold Start — це компроміс" icon="i-heroicons-clock"}
...
::card{title="IAM — найменші привілеї" icon="i-heroicons-shield-check"}
...
::card{title="Observability з CloudWatch" icon="i-heroicons-chart-bar"}
...
::card{title="Вартість = час × пам'ять" icon="i-heroicons-currency-dollar"}
...
::card{title="IaC для всього" icon="i-heroicons-code-bracket"}
