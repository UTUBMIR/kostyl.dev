...
### Крок 1: Створення структури папок та ініціалізація Git

Створіть кореневу директорію проєкту, базову структуру каталогів, а також ініціалізуйте Git-репозиторій:
...
Також створіть файл `taskforge/.gitignore`, щоб не додавати у контроль версій віртуальне середовище, тимчасові файли та кеш:
...
> [!NOTE]
> Секція `[tool.hatch.build.targets.wheel]` із параметром `packages = ["app"]` є обов'язковою для нашої структури. За замовчуванням збирач `hatchling` очікує, що назва головного пакета збігається з назвою проєкту (`taskforge`), або шукає вихідні файли в каталозі `src/`. Оскільки ми назвали папку з кодом додатка `app`, нам потрібно явно вказати збирачу Hatch, яку саме директорію включати в дистрибутив. Без цього налаштування інструменти на кшталт `pip install -e .` чи `uv sync` не знайдуть код додатка, через що імпорти не працюватимуть.
