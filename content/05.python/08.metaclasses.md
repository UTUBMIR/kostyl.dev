...
::debugger-view{title="Послідовність виконання при class MyClass(Base, metaclass=Meta): ... / obj = MyClass()"}

```
[1] type.__prepare__(mcs, name, bases)          # (опційно) створює словник namespace
[2] <виконується тіло класу в namespace>
[3] Meta.__new__(mcs, name, bases, namespace)   # Створення об'єкта класу
[4] Meta.__init__(cls, name, bases, namespace)  # Ініціалізація об'єкта класу
─── Клас MyClass готовий ───────────────────────────────────────────────
[5] MyClass.__new__(cls, *args, **kwargs)        # Створення екземпляра
[6] MyClass.__init__(obj, *args, **kwargs)       # Ініціалізація екземпляра
