...
```mermaid
graph TD
    A["CSS Розміри"] --> B["Абсолютні<br/>px, pt, cm, mm"]
    A --> C["Відносні шрифтові<br/>em, rem, ch, ex"]
    A --> D["Viewport<br/>vw, vh, dvh, svh, lvh"]
    A --> E["Контейнерні<br/>cqw, cqh, cqi"]
    A --> F["Intrinsic keywords<br/>min-content, max-content<br/>fit-content, auto"]
    A --> G["Функції<br/>calc(), min(), max()<br/>clamp()"]
    A --> H["Grid-специфічні<br/>fr"]
...
```mermaid
graph LR
    A["1px CSS"] --> B["DPR 1<br/>(звичайний монітор)<br/>1 фіз. піксель"]
    A --> C["DPR 2<br/>(Retina)<br/>4 фіз. пікселі"]
    A --> D["DPR 3<br/>(iPhone Pro)<br/>9 фіз. пікселів"]
...
```mermaid
graph LR
  A["Сторінка завантажена<br/>(адресний рядок видимий)"] -->|"100vh = вся висота<br/>включно з адрес-баром"| B["Hero виходить<br/>за межі екрана ❌"]
  C["Після прокручування<br/>(адресний рядок прихований)"] -->|"100vh збільшується"| D["Hero 'стрибає' ❌"]
