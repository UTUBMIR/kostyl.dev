...
```mermaid
graph LR
    A["position: static<br/>(нормальний потік)"] --> B["position: relative<br/>(відносно свого місця)"]
    B --> C["position: absolute<br/>(відносно позиціонованого предка)"]
    B --> D["position: fixed<br/>(відносно viewport)"]
    B --> E["position: sticky<br/>(hybrid: relative + fixed)"]
...
```mermaid
graph TD
    Root["Root context (html)<br/>z-index: auto"] --> A["Контекст A<br/>z-index: 1"]
    Root --> B["Контекст B<br/>z-index: 2"]
    A --> A1["Елемент A1<br/>z-index: 9999"]
    A --> A2["Елемент A2<br/>z-index: 1"]
    B --> B1["Елемент B1<br/>z-index: 1"]
...
```mermaid
flowchart TD
    Start["Потрібно позиціонувати елемент"] --> Q1{"Елемент повинен<br/>йти с потоку?"}
    Q1 -->|Ні| Rel["position: relative<br/>• Дрібне зміщення<br/>• Системa відліку для absolute"]
    Q1 -->|Так| Q2{"Відносно чого?"}
    Q2 -->|Предка| Abs["position: absolute<br/>• Відносно найближчого<br/>  позиціонованого предка<br/>• Tooltips, badges, menus"]
    Q2 -->|Viewport| Q3{"Прокручується<br/>зі сторінкою?"}
    Q3 -->|Ні| Fix["position: fixed<br/>• Завжди в viewport<br/>• Cookie banner, chat"]
    Q3 -->|Так потім ні| Stic["position: sticky<br/>• Scroll hybrid<br/>• Sticky header, sections"]
```
