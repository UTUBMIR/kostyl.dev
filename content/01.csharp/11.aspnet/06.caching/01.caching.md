...
graph TD
    CLIENT["🌐 Браузер / CDN"]
    RC["Response Cache<br/>(Cache-Control headers)<br/>Браузер або CDN дає відповідь сам"]
    OC["Output Cache<br/>(Server-side HTTP cache)<br/>Сервер кешує готову HTTP-відповідь"]
    MC["IMemoryCache<br/>(RAM сервера)<br/>Хвилини, швидкий, один інстанс"]
    DC["IDistributedCache<br/>(Redis / SQL Server)<br/>Мілісекунди + мережа, кілька інстансів"]
    DB["🗄️ База даних"]
...
```mermaid
graph LR
    BR["🌐 Браузер"] -->|"max-age=300<br/>(5 хвилин)"| BR_CACHE["Browser Cache"]
    BR -->|Cache miss| CDN["☁️ CDN<br/>(Cloudflare)"]
    CDN -->|"s-maxage=3600<br/>(1 година)"| CDN_CACHE["CDN Cache"]
    CDN -->|Cache miss| LB["⚖️ Load Balancer"]

    LB --> S1["Сервер 1"]
    LB --> S2["Сервер 2"]

    S1 --> OC["Output Cache<br/>(Server-side)"]
    OC -->|Cache miss| APP["Endpoint Handler"]

    APP --> MC["IMemoryCache<br/>(L1, ~0.01 мс)"]
    APP --> RC["Redis<br/>(L2, ~2 мс)"]
    APP --> DB["PostgreSQL<br/>(~5-50 мс)"]
