...
```mermaid
graph TD
    A["📝 Test Author<br/>Пише тести з атрибутами"] --> B["🔍 Test Discovery<br/>Scanує сборки, знаходить тести"]
    B --> C["📋 Test Plan<br/>Створює список тестів для виконання"]
    C --> D["⚙️ Test Runner<br/>Виконує тести ізольовано"]
    D --> E1["🟢 PASS"]
    D --> E2["🔴 FAIL"]
    D --> E3["⏭️ SKIP"]
    E1 & E2 & E3 --> F["📊 Reporter<br/>Формує XML/JSON/HTML звіт"]
    F --> G["🖥️ IDE / CI<br/>Відображає результати"]
...
graph TD
    subgraph "Тестовий фреймворк"
        FW["xUnit / NUnit / MSTest<br/>Тест-runner + Discovery + Assertions"]
    end

    subgraph "Assertions"
        FA["FluentAssertions<br/>Читабельні assertions"]
        SH["Shouldly<br/>Альтернатива FluentAssertions"]
    end

    subgraph "Тестові дані"
        AF["AutoFixture<br/>Автогенерація об'єктів"]
        BF["Bogus<br/>Realistic fake data"]
        BL["Fixture Builder<br/>(власні builder'и)"]
    end

    subgraph "Мокування"
        MQ["Moq<br/>Найпопулярніший mock-фреймворк"]
        NS["NSubstitute<br/>Лаконічний синтаксис"]
        FQ["FakeItEasy<br/>Альтернатива"]
    end

    subgraph "Інфраструктура"
        TC["Testcontainers<br/>Docker для тестів"]
        WM["WireMock.Net<br/>HTTP mocking server"]
        RS["Respawn<br/>DB cleanup"]
    end

    subgraph "Якість тестів"
        CV["Coverlet<br/>Code Coverage"]
        SR["Stryker.NET<br/>Mutation Testing"]
        RG["ReportGenerator<br/>HTML Coverage Reports"]
    end
