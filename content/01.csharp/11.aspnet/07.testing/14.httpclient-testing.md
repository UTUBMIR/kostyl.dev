...
```mermaid
graph LR
    A["HttpClient<br/>.GetAsync()"] -->|"передає HttpRequestMessage"| B["HttpMessageHandler<br/>(абстрактний)"]
    B --> C["HttpClientHandler<br/>(реальна реалізація)"]
...
```mermaid
graph LR
    A["HttpClient"] --> B["LoggingHandler<br/>(DelegatingHandler)"]
    B --> C["RetryHandler<br/>(DelegatingHandler)"]
    C --> D["AuthHandler<br/>(DelegatingHandler)"]
    D --> E["HttpClientHandler<br/>(кінцевий обробник)"]
