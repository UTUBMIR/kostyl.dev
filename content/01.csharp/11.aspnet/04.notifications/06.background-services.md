...
```mermaid
graph TB
    APP["ASP.NET Core Host"] --> HTTP["HTTP Pipeline<br/>(звичайні запити)"]
    APP --> SIM["NotificationSimulator<br/>(симулює події кожні N секунд)"]
    APP --> QUEUE["QueuedHostedService<br/>(обробляє чергу задач)"]
    APP --> CLEAN["DatabaseCleanupService<br/>(очищає стари дані щоночі)"]

    HTTP -->|"Enqueue(task)"| Q[("Channel&lt;T&gt;<br/>Черга задач")]
...
stateDiagram-v2
    ...
    Running --> Stopping: app.Lifetime.StopApplication()<br/>Ctrl+C / docker stop
    Stopping --> Stopped: StoppingToken скасовано<br/>ExecuteAsync() завершено<br/>StopAsync() викликано
    Stopped --> [*]
```
