...
graph LR
    subgraph "ASP.NET Core Process"
        C["🖥️ Controller / Endpoint<br/>(Hangfire Client)"]
        S["⚙️ Hangfire Server<br/>(BackgroundService)"]
        W1["👷 Worker 1"]
        W2["👷 Worker 2"]
        W3["👷 Worker N"]
    end

    DB[("🗄️ Storage<br/>PostgreSQL / SQL Server / Redis")]

    C -->|"Enqueue / Schedule<br/>(JSON serialize)"| DB
    DB -->|"Dequeue<br/>(polling)"| S
    S --> W1
    S --> W2
    S --> W3
