...
```mermaid
graph LR
    A["POST /notifications<br/>(створення нотифікації)"] -->|"Publish(notification)"| B[("Channel&lt;T&gt;<br/>(черга повідомлень)")]
    B -->|"ReadAllAsync()"| C["SSE Handler<br/>(GET /notifications/stream)"]
    C -->|"text/event-stream"| D["🌐 Браузер<br/>(EventSource)"]
