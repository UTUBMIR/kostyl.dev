...
```mermaid
graph TB
    TRIGGER["🚀 POST /campaigns/send<br/>Hangfire: StartCampaignAsync(campaignId)"]

    TRIGGER --> BATCH1["📦 Batch 1<br/>(chatIds 1–1000)<br/>Hangfire Job"]
    TRIGGER --> BATCH2["📦 Batch 2<br/>(chatIds 1001–2000)<br/>Hangfire Job"]
    TRIGGER --> BATCH3["📦 Batch 3<br/>(chatIds 2001–3000)<br/>Hangfire Job"]
    TRIGGER --> BATCHN["📦 Batch N<br/>(chatIds N...)<br/>Hangfire Job"]

    BATCH1 --> W1["⚙️ Worker 1<br/>25 паралельних<br/>Telegram запитів"]
    BATCH2 --> W2["⚙️ Worker 2<br/>25 паралельних<br/>Telegram запитів"]
    BATCH3 --> W3["⚙️ Worker 3<br/>25 паралельних<br/>Telegram запитів"]
...
```mermaid
flowchart TD
    MSG["📨 NotificationMessage<br/>Priority = ?"]

    MSG --> CRIT{{"Critical?"}}
    CRIT -- Так --> ALL["SMS + Telegram + Email<br/>ВСІ одночасно"]

    CRIT -- Ні --> HIGH{{"High?"}}
    HIGH -- Так --> HIGH_CH["Telegram + Email<br/>(якщо є)"]

    HIGH -- Ні --> NORM{{"Normal?"}}
    NORM -- Так --> FIRST["Перший доступний:<br/>Telegram → Push → Email"]
