...
```mermaid
graph LR
    A["HTTP Request<br/>(JSON Body)"] --> B["ModelBinder<br/>(Deserialize)"]
    B --> C["RegisterRequest DTO"]
    C --> D["RegisterRequestValidator<br/>(AbstractValidator)"]
    D -->|"Valid"| E["Controller / Handler"]
    D -->|"Invalid"| F["400 Bad Request<br/>(ProblemDetails)"]
