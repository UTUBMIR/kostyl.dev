...
```mermaid
graph TD
    CR["🏗️ Composition Root<br/>(Program.cs)"]
    CT["📦 IoC Container"]
    OS["OrderService"]
    ES["IEmailService<br/>→ SmtpEmailService"]
    OR["IOrderRepository<br/>→ SqlOrderRepository"]
    PG["IPaymentGateway<br/>→ StripePaymentGateway"]
    LG["ILogger<br/>→ ConsoleLogger"]

    CR -->|"Реєструє всі типи"| CT
    CT -->|"Автоматично створює<br/>і підставляє залежності"| OS
    OS -->|залежить від| ES
    OS -->|залежить від| OR
    OS -->|залежить від| PG
    ES -->|залежить від| LG
