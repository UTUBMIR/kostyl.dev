...
```mermaid
graph TD
    Problem["Спільний Стан<br/>⚠️"] --> RC["Race Condition<br/>Check-then-Act"]
    Problem --> DR["Data Race<br/>i++ без sync"]
    Problem --> VP["Visibility<br/>CPU Cache"]
    Problem --> MR["Memory<br/>Reordering"]
    Problem --> TR["Torn Read<br/>64-bit на 32-bit"]

    RC --> Lock["🔒 lock / Monitor<br/>Тема 06"]
    DR --> LockOrInterlocked["🔒 lock / Interlocked<br/>Тема 06"]
    VP --> Volatile["⚡ volatile / Interlocked<br/>Тема 05a (ця)"]
    MR --> VolatileBarrier["⚡ volatile / MemoryBarrier<br/>Тема 05a"]
    TR --> InterlockedOrLock["🔒 Interlocked / lock<br/>Тема 05a / 06"]

    Lock --> Sem["🚦 Semaphore<br/>Тема 07"]
    Lock --> RWL["📖 ReaderWriterLock<br/>Тема 07"]
    LockOrInterlocked --> LF["⚙️ Lock-Free<br/>Algorithms"]
