...
graph TD
    ...
    B -- "Show()" --> C["SettingsWindow відкривається<br/>Виконання продовжується одразу<br/>Обидва вікна активні"]
    B -- "ShowDialog()" --> D["ConfirmDialog відкривається<br/>Виконання зупиняється тут<br/>Gorlovка лише для ConfirmDialog"]
    D -- "Закрито — DialogResult=true" --> E["Виконання відновлюється<br/>Result: true"]
    D -- "Закрито — DialogResult=false" --> F["Виконання відновлюється<br/>Result: false"]
