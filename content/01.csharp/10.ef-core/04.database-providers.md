...
graph TD
    subgraph "Ваш код"
        A["📝 LINQ / DbContext API<br/>(не знає про СУБД)"]
    end

    subgraph "EF Core Core"
        B["🌳 Query Pipeline<br/>(Expression Trees)"]
        C["🔄 IQuerySqlGenerator<br/>(абстрактний генератор SQL)"]
        D["📐 IMigrationsModelDiffer<br/>(абстрактний дифер міграцій)"]
        E["🔌 IRelationalConnection<br/>(абстрактне з'єднання)"]
    end

    subgraph "Провайдери (замінні)"
        F["🐘 Npgsql<br/>(PostgreSQL)"]
        G["🏢 SqlServer<br/>(Microsoft SQL Server)"]
        H["📁 Sqlite<br/>(SQLite)"]
        I["🐬 Pomelo<br/>(MySQL / MariaDB)"]
    end
