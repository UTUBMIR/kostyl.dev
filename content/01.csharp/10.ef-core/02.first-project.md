...
```mermaid
graph LR
    A["📝 LINQ-вираз<br/>context.Books.Where(...)"]
    B["🌳 Expression Tree<br/>(C#-дерево виразу)"]
    C["🔄 Query Compiler<br/>(EF Core внутрішній механізм)"]
    D["💬 SQL-рядок<br/>SELECT ... WHERE ..."]
    E["🔌 Database Provider<br/>SQLite / PostgreSQL / ..."]
    F["📊 DbDataReader<br/>(сирі рядки результату)"]
    G["✨ Матеріалізація<br/>(C#-об'єкти)"]
    H["👁️ Change Tracker<br/>(реєстрація об'єктів)"]
    I["📦 Результат<br/>List<Book>"]
