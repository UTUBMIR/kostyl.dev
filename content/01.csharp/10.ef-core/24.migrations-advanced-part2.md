...
graph LR
    subgraph "Деплой A: Expand"
        A1["Додати нові<br/>Stовпці/Таблиці<br/>(nullable)"]
        A2["Новий код пише<br/>у ОБИДВА місця<br/>(старе і нове)"]
    end

    subgraph "Деплой B: Migrate"
        B1["Заповнити нові<br/>стовпці даними<br/>(data migration)"]
        B2["Новий код читає<br/> лише з нових місць"]
    end

    subgraph "Деплой C: Contract"
        C1["Видалити старі<br/>стовпці/Таблиці"]
        C2["Код чистий:<br/>лише нова схема"]
    end
