...
```mermaid
flowchart TD
    DEV["👨‍💻 Розробник змінює<br/>C# Model"]
    ADD["dotnet ef migrations add<br/>MigrationName"]
    FILES["Файли у Migrations/<br/>*.cs + *.Designer.cs<br/>ModelSnapshot.cs"]
    GIT["Git commit + push<br/>(включаючи Migrations/)"]
    CI["CI Pipeline<br/>build + test"]

    SCRIPT["dotnet ef migrations script<br/>--idempotent<br/>--output migrations.sql"]
    BUNDLE["dotnet ef migrations bundle<br/>--self-contained<br/>--output efbundle"]

    DBA["DBA Review<br/>migrations.sql"]
    APPROVE{Approved?}

    STAGING["Staging Database<br/>migrations.sql/efbundle"]
    TEST["Integration Tests<br/>на staging"]
    PASS{Tests pass?}

    PROD["Production Database<br/>migrations.sql/efbundle"]
    DEPLOY["Deploy Application<br/>(нова версія)"]
    HISTORY["__EFMigrationsHistory<br/>логує застосоване"]
