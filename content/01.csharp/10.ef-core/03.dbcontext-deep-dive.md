...
graph TD
    subgraph "DbContext"
        CT["🔍 ChangeTracker<br/>(відстеження змін)"]
        SM["📋 StateManager<br/>(стан сутностей)"]
        QC["⚙️ QueryCompiler<br/>(LINQ → SQL)"]
        DB["🔌 DatabaseFacade<br/>(з'єднання, транзакції, міграції)"]
        MD["📐 IModel<br/>(скомпільована модель)"]
    end
...
```mermaid
graph LR
    A["🔨 new DbContext()<br/>ChangeTracker: порожній<br/>Model: завантажена з кешу"]
    B["📖 Перший запит<br/>Відкриття з'єднання<br/>SQL виконується<br/>Об'єкти в ChangeTracker"]
    C["✏️ Зміни<br/>ChangeTracker відстежує<br/>Entity states змінюються"]
    D["💾 SaveChanges()<br/>DetectChanges<br/>SQL генерація<br/>Транзакція<br/>State reset"]
    E["🗑️ Dispose()<br/>ChangeTracker очищено<br/>З'єднання в пул"]
