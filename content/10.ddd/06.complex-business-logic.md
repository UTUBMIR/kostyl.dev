...
classDiagram
    ...
    note for Color "Ідентифікується значеннями<br/>Без явного ID<br/>Незмінний"
```
...
```mermaid
flowchart TD
    Start[Бізнес-об'єкт] --> Q1{Чи потрібен<br/>поле ID?}
    Q1 -->|Ні| VO[Value Object]
    Q1 -->|Так| Q2{Чи має власні<br/>бізнес-правила?}

    Q2 -->|Ні| Child[Entity<br/>всередині aggregate]
    Q2 -->|Так| Q3{Чи потрібна<br/>конзистентність<br/>з іншими об'єктами?}

    Q3 -->|З батьківським об'єктом| Child
    Q3 -->|Незалежна| AggRoot[Aggregate Root]

    Q3 -->|Залежить від сценарію| Q4{Які кількісні<br/>співрозмірності?}
