...
flowchart TB
    ...
    Scalar --> ScalarDesc["Повертають одне значення<br/>(число, рядок, дата)"]
    
    Table --> Inline[Вбудовані табличні]
    Table --> Multi[Багатооператорні]
    
    Inline --> InlineDesc["Один SELECT<br/>Висока продуктивність"]
    Multi --> MultiDesc["Багато операторів<br/>Створюють таблицю"]
