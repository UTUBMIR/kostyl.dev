...
Типовий стек для Node.js API: швидкий Fastify як HTTP-фреймворк, MongoDB як документна база даних та Mongo Express як вебінтерфейс для перегляду даних.
...
# З Mongo Express для перегляду даних
...
1. Compose запускає MongoDB та чекає health check (`mongosh ping`)
2. Лише після цього запускається `api` — нема ризику падіння через "БД не готова"
3. `mongo-express` запускається лише з профілем `debug` — не заважає у production
4. `node_modules` захищені анонімним volume — не перезаписуються bind mount

::tip
**Hot-reload для розробки:** Замініть `node index.js` на `npx nodemon index.js` — Fastify буде перезапускатися при кожній зміні файлів у `./app`.
::
...
Класичний C# стек: Web API на ASP.NET Core 8, PostgreSQL як база даних та pgAdmin як вебінтерфейс для DBA.
...
**`api/Program.cs` (мінімальний Web API):**
...
```bash
# Запустити API + PostgreSQL
docker compose up -d

# З pgAdmin для перегляду бази
docker compose --profile debug up -d
```
...
# Логи API (бачимо EF Core migrations)
...
- `condition: service_healthy` — API стартує **лише після** того, як PostgreSQL пройде `pg_isready` перевірку
- `restart: on-failure` — якщо API впав до готовності БД (race condition) — перезапуститься автоматично
- `pgadmin` з профілем `debug` — зручний UI для розробника, але не запускається у production
- Multi-stage Dockerfile — `sdk` тільки для збірки, фінальний образ на `aspnet` (менший розмір)

::tip
**EF Core міграції:** Виклик `db.Database.Migrate()` у `Program.cs` автоматично застосовує міграції при старті контейнера. Це зручно для development — не потрібно запускати міграції вручну.
::
...
Сучасний JS full-stack: Next.js як єдиний сервіс (frontend + API routes), PostgreSQL для даних та Redis для кешування сесій і відповідей.
...
**`web/app/api/posts/route.ts` (приклад API Route з кешуванням):**
...
export async function GET() {
  ...
  // Якщо кешу нема — читаємо з БД
  const posts = await prisma.post.findMany({
    orderBy: { createdAt: 'desc' },
    take: 20,
  })

  // Кешуємо на 60 секунд
...
# З Redis Insight для моніторингу кешу
docker compose --profile debug up -d

# Переглянути логи Next.js
...
```bash
# Перший запит — читає з PostgreSQL
curl -v http://localhost:3000/api/posts
# X-Cache: MISS

# Другий запит — з Redis кешу
curl -v http://localhost:3000/api/posts
# X-Cache: HIT

# Redis Insight UI (моніторинг ключів)
...
```
Браузер → Next.js (app/) → API Route
                              ↓
                         Redis Cache ← HIT → відповідь
                              ↓ MISS
                         PostgreSQL → зберегти в Redis → відповідь
```
...
| Компонент | Роль | Порт |
|-----------|------|------|
| Next.js | Frontend + API Routes | 3000 |
| PostgreSQL | Persistent storage | (внутрішній) |
| Redis | Cache + Sessions | (внутрішній) |
| Redis Insight | Debug UI | 5050 (debug profile) |

::tip
**Production deployment:** Замініть `target: development` на `target: production` у `docker-compose.prod.yml` — Next.js збереться у standalone-режимі, що значно зменшує розмір образу і час старту.
::
