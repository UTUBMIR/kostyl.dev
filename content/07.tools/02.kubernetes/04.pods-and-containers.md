...
![kubernetes pod structure containers shared network volumes](/images/tools/kubernetes/pods-and-containers/01.png){.diagram-img}
...
package "Kubernetes Node" #f8f9fa {
    package "Pod (IP: 10.244.1.5)" #e3f2fd {
        ...
        storage "Volume 1\n(emptyDir)" as v1 #ffe0b2
        storage "Volume 2\n(configMap)" as v2 #ffe0b2
        
        c1 -down-> v1 : mount /data
        c2 -down-> v1 : mount /data
        c2 -down-> v2 : mount /config
    }
    
    note right of "Pod (IP: 10.244.1.5)"
        Спільна мережа:
        - Один IP на Pod
        - Контейнери → localhost
        - Спільні порти
    end note
    ...
    note bottom of v1
        Спільні volumes:
        - emptyDir (тимчасові)
        - configMap (конфігурації)
        - secret (секрети)
        - persistentVolume (постійні)
    end note
}

c1 <-[#green]-> c2 : localhost\n(спільна мережа)

note bottom of "Kubernetes Node"
    Всі контейнери Pod:
    ✓ Завжди на одному вузлі
    ✓ Запускаються разом
    ✓ Зупиняються разом
end note
@enduml
...
::field{name="Аналогія для запам'ятовування" type="порада"}
Уявіть життя Pod як **політ літака**:
- **Pending**: Пасажири в черзі на посадку, літак готується до зльоту.
- **Running**: Літак у небі, двигуни працюють (контейнери виконуються).
- **Succeeded**: М'яка посадка, місія завершена (скрипт відпрацював і вимкнувся).
- **Failed**: Аварійна ситуація, двигун відмовив або скінчилося пальне.
::
...
stateDiagram-v2
    ...
    classDef succeeded fill:#3b82f6,stroke:#1d4ed8,color:#fff
    classDef failed fill:#ef4444,stroke:#b91c1c,color:#fff

    [*] --> Pending: "kubectl apply"
    Pending --> Running: "Scheduler призначив вузол (Kubelet запустив контейнери)"
    Pending --> Failed: "Помилка завантаження образу або недостатньо ресурсів"
    Running --> Succeeded: "Контейнери завершились успішно (restartPolicy Never)"
    Running --> Failed: "Контейнер впав (exit code != 0)"
    Running --> Running: "Контейнер перезапущено (restartPolicy Always)"
    Failed --> Running: "Контейнер перезапущено (restartPolicy OnFailure)"
    Succeeded --> [*]
    Failed --> [*]
...
![kubernetes init containers sequential execution workflow](/images/tools/kubernetes/pods-and-containers/02.jpeg){.diagram-img}
...
participant "Init Container 1\n(wait-for-db)" as init1
participant "Init Container 2\n(run-migrations)" as init2
...
init2 -> init2: Застосувати зміни до БД
init2 --> k8s: Exit code 0 (успіх)
deactivate init2

k8s -> main: Запустити
activate main
main -> main: Застосунок працює
note right of main
  Основний контейнер
...
::tip
**Аналогія: Відкриття ресторану**
Уявіть, що ви відкриваєте новий ресторан. Перед тим як запросити гостей (**запустити основний контейнер**), ви повинні виконати підготовчі кроки (**Init-контейнери**):
1. **Завезти продукти** (Init 1).
2. **Налаштувати касовий апарат** (Init 2).
3. **Провести генеральне прибирання** (Init 3).

Тільки після того, як ви успішно завершили всі ці етапи, ви відкриваєте двері для клієнтів. Якщо на будь-якому етапі сталася проблема (наприклад, не привезли продукти), ресторан не відкриється.
::
...
::tip
**Аналогія: Рюкзак мандрівника**
Sidecar — це як **рюкзак у мандрівника**. Мандрівник (**основний контейнер**) просто йде своїм маршрутом (виконує бізнес-логіку). Але в рюкзаку (**sidecar**) лежить вода, мапа та ліхтарик. Рюкзак забезпечує мандрівника всім необхідним (збирає логи, проксіює трафік), не заважаючи йому йти.
::

### Типові сценарії використання sidecar

![kubernetes sidecar pattern logging proxy configuration](/images/tools/kubernetes/pods-and-containers/03.png){.diagram-img}
...
sidecar -right-> prom : export metrics
...
![kubernetes pod volumes emptydir configmap secret hostpath](/images/tools/kubernetes/pods-and-containers/04.png){.diagram-img}
...
cloud "Node Filesystem" as node

cm <-- api : read-only
sec <-- api : read-only
hp <-- node : direct access

note right of empty
    emptyDir:
    "✓" Швидкий обмін даними
    "✗" Не зберігається після видалення Pod
end note
...
note right of sec
    ...
    "✓" Base64 кодування
    "✓" Шифрування в etcd
    "✗" Не для великих даних
end note

note right of hp
    hostPath:
    "⚠️" Порушує ізоляцію
    "⚠️" Тільки для системних Pod
end note
@enduml
```
::
...
::tip
**Аналогія: Спільна папка**
Volumes у Pod — це як **спільна папка** на сервері або **зовнішній жорсткий диск**. Вони дозволяють контейнерам обмінюватися файлами так само легко, як ви передаєте флешку другу, або зберігати дані, щоб вони не зникли після перезапуску контейнера.
::
...
::tip
**Аналогія: Тимчасова дошка**
`emptyDir` — це як **тимчасова дошка** в офісі: ви можете на ній писати, поки йде нарада (**поки працює Pod**). Але як тільки нарада завершується і всі йдуть додому (**Pod видаляється**), дошку витирають дочиста.
::
