...
**Завдання 1.** Створіть Pod з образом `nginx:1.27`. Використайте `kubectl port-forward` для доступу до Nginx на порту 8080 вашої машини. Відкрийте браузер та перевірте, що Nginx працює.

**Завдання 2.** Створіть Pod з двома контейнерами: `nginx:1.27` та `busybox:1.36` (з командою `sleep 3600`). Використайте `kubectl exec` для входу в контейнер `busybox` та виконайте `wget -O- localhost:80`. Поясніть, чому це працює (підказка: спільна мережа).

**Завдання 3.** Створіть Pod з `restartPolicy: Never` та контейнером, який завершується з помилкою (наприклад, `command: ["sh", "-c", "exit 1"]`). Перевірте фазу Pod через `kubectl get pod`. Поясніть, чому Pod не перезапускається.

### Рівень 2: Практичний

**Завдання 4.** Модифікуйте TodoApi з прикладу: додайте ендпоінт `PUT /api/todos/{id}` для оновлення задачі. Перезберіть образ, оновіть Pod (видаліть старий та створіть новий) та протестуйте новий ендпоінт.

**Завдання 5.** Створіть Pod з volume типу `emptyDir`. Один контейнер пише файл у цей volume (`echo "Hello" > /data/message.txt`), другий контейнер читає його (`cat /data/message.txt`). Перевірте через `kubectl logs`, що обидва контейнери бачать один і той самий файл.

**Завдання 6.** Створіть Pod з ресурсними обмеженнями: `requests: {memory: "64Mi", cpu: "100m"}`, `limits: {memory: "128Mi", cpu: "200m"}`. Використайте `kubectl describe pod` для перевірки, що обмеження застосовані. Використайте `kubectl top pod` для перегляду реального використання ресурсів.

### Рівень 3: Дослідницький

**Завдання 7.** Створіть Pod з контейнером, який має `resources.limits.memory: "64Mi"` та виконує команду, що споживає більше пам'яті. Використайте `kubectl describe pod` для перегляду причини завершення контейнера. Знайдіть у виводі `OOMKilled` (Out Of Memory Killed) та поясніть, що це означає.

**Завдання 8.** Експериментуйте з `restartPolicy`. Створіть три Pod з різними політиками (`Always`, `OnFailure`, `Never`) та контейнером, який завершується з помилкою через 10 секунд. Спостерігайте за поведінкою через `kubectl get pods -w` (watch mode). Поясніть різницю у поведінці.

**Завдання 9.** Створіть ConfigMap з файлом конфігурації (наприклад, JSON з налаштуваннями). Змонтуйте цей ConfigMap як volume у Pod. Використайте `kubectl exec` для перевірки, що файл доступний всередині контейнера. Змініть ConfigMap та перевірте, чи оновився файл у контейнері (підказка: може знадобитися час на синхронізацію).
