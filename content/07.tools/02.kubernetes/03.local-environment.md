...
![kubernetes local development tools comparison minikube kind k3s](/images/tools/kubernetes/local-environment/01.png){.diagram-img}
...
![minikube start process architecture docker container kubernetes components](/images/tools/kubernetes/local-environment/02.png){.diagram-img}
...
::card{title="kubectx" icon="i-heroicons-arrows-right-left"}
Дозволяє перемикати кластери (контексти) однією короткою командою. Наприклад: `kubectx minikube`.
::

::card{title="kubens" icon="i-heroicons-tag"}
...
![kubernetes pod creation workflow kubectl apiserver scheduler kubelet](/images/tools/kubernetes/local-environment/03.jpg){.diagram-img}
