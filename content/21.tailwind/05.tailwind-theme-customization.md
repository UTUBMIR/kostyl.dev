...
<body class="p-6 bg-slate-50 font-sans">
    <p class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">Результат першого @theme</p>
    <div class="bg-white p-6 rounded-card shadow-md border border-slate-100 max-w-sm">
        <h3 class="font-display font-bold text-2xl text-brand mb-2">Display Satoshi</h3>
        <p class="text-slate-600 text-sm leading-relaxed mb-4">Ця картка використовує заокруглення <code class="bg-slate-100 px-1 py-0.5 rounded text-xs">rounded-card</code> (12px) та колір бренду <code class="bg-slate-100 px-1 py-0.5 rounded text-xs text-brand">text-brand</code>.</p>
        <button class="w-full py-2 bg-brand text-white font-semibold rounded-lg text-sm shadow hover:opacity-95 transition-opacity">
            Active Button
        </button>
    </div>
</body>
...
<body class="p-6 bg-slate-50 font-sans flex flex-col items-center">
    <div id="demo-box" class="w-full max-w-sm p-6 bg-[var(--color-brand)] text-white rounded-[var(--radius-brand)] shadow-lg transition-all duration-500">
        <h4 class="font-bold text-xl mb-2">Динамічний токен</h4>
        <p class="text-sm opacity-90 mb-4">Колір фону та радіус цієї картки залежать від CSS-змінних <code class="bg-black/20 px-1 py-0.5 rounded">--color-brand</code> та <code class="bg-black/20 px-1 py-0.5 rounded">--radius-brand</code>.</p>
        <div class="flex gap-2">
            <button onclick="setBrand('purple')" class="flex-1 py-1.5 bg-white/20 hover:bg-white/30 text-xs font-semibold rounded transition-colors">
                Purple (270°)
            </button>
            <button onclick="setBrand('green')" class="flex-1 py-1.5 bg-white/20 hover:bg-white/30 text-xs font-semibold rounded transition-colors">
                Green (145°)
            </button>
        </div>
    </div>
    <script>
        function setBrand(color) {
            const root = document.getElementById('demo-box');
            if (color === 'purple') {
                root.style.setProperty('--color-brand', 'oklch(0.6 0.2 270)');
            } else {
                root.style.setProperty('--color-brand', 'oklch(0.7 0.18 145)');
            }
        }
    </script>
</body>
...
<body class="p-6 bg-slate-50 font-sans flex flex-col items-center">
    <div class="w-full max-w-md bg-white p-6 rounded-2xl shadow-md border border-slate-100">
        <p class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Інтерактивний OKLCH Explorer</p>
        
        <!-- Color Display Swatch -->
        <div id="swatch" class="h-32 rounded-xl mb-6 shadow-inner transition-colors duration-150 flex flex-col justify-end p-4 text-white font-mono text-sm font-semibold" style="background-color: oklch(0.6 0.2 270)">
            <span id="oklch-text" class="bg-black/30 backdrop-blur px-2.5 py-1 rounded-lg w-fit">oklch(0.6 0.2 270)</span>
        </div>
        ...
        <div class="space-y-4">
            ...
            <div>
                <div class="flex justify-between text-xs font-bold text-slate-500 mb-1">
                    <span>L — Lightness (Яскравість)</span>
                    <span id="l-val" class="font-mono">0.6</span>
                </div>
                <input type="range" id="slider-l" min="0" max="1" step="0.01" value="0.6" class="w-full h-1.5 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-violet-600">
            </div>

            <!-- Chroma Slider -->
            <div>
                <div class="flex justify-between text-xs font-bold text-slate-500 mb-1">
                    <span>C — Chroma (Насиченість)</span>
                    <span id="c-val" class="font-mono">0.2</span>
                </div>
                <input type="range" id="slider-c" min="0" max="0.37" step="0.01" value="0.2" class="w-full h-1.5 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-violet-600">
            </div>

            <!-- Hue Slider -->
            <div>
                <div class="flex justify-between text-xs font-bold text-slate-500 mb-1">
                    <span>H — Hue (Відтінок)</span>
                    <span id="h-val" class="font-mono">270°</span>
                </div>
                <input type="range" id="slider-h" min="0" max="360" step="1" value="270" class="w-full h-1.5 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-violet-600">
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        const oklchText = document.getElementById('oklch-text');
        ...
        function updateColor() {
            ...
            swatch.style.backgroundColor = oklchColor;
            oklchText.innerText = oklchColor;
        }
...
<style>
    ...
    .bp-phone-active, .bp-tablet-active, .bp-laptop-active, .bp-desktop-active, .bp-wide-active {
        display: none;
    }
    .bp-default-active { display: inline-block; }

    /* Phone: >= 360px (22.5rem) */
    @container (min-width: 360px) {
        .responsive-card { background-color: #f0fdf4; border-color: #bbf7d0; } /* green-50 */
        .bp-badge-phone { background-color: #10b981; color: white; }
        .bp-phone-active { display: inline-block; }
        .bp-default-active { display: none; }
    }

    /* Tablet: >= 768px (48rem) */
    @container (min-width: 768px) {
        .responsive-card { background-color: #eff6ff; border-color: #bfdbfe; } /* blue-50 */
        .bp-badge-tablet { background-color: #3b82f6; color: white; }
        .bp-tablet-active { display: inline-block; }
        .bp-phone-active { display: none; }
    }

    /* Laptop: >= 1024px (64rem) */
    @container (min-width: 1024px) {
        .responsive-card { background-color: #faf5ff; border-color: #e9d5ff; } /* purple-50 */
        .bp-badge-laptop { background-color: #8b5cf6; color: white; }
        .bp-laptop-active { display: inline-block; }
        .bp-tablet-active { display: none; }
    }

    /* Desktop: >= 1280px (80rem) */
    @container (min-width: 1280px) {
        .responsive-card { background-color: #fff7ed; border-color: #fed7aa; } /* orange-50 */
        .bp-badge-desktop { background-color: #f97316; color: white; }
        .bp-desktop-active { display: inline-block; }
        .bp-laptop-active { display: none; }
    }

    /* Wide: >= 1600px (100rem) */
    @container (min-width: 1600px) {
        .responsive-card { background-color: #fff1f2; border-color: #fecdd3; } /* rose-50 */
        .bp-badge-wide { background-color: #f43f5e; color: white; }
        .bp-wide-active { display: inline-block; }
        .bp-desktop-active { display: none; }
    }
</style>
</head>
<body class="p-6 bg-slate-50 font-sans flex flex-col items-center">
    <div class="w-full max-w-3xl">
        <p class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">Симулятор адаптивних варіантів (Breakpoints)</p>
        
        <!-- Viewport Controls -->
        <div class="flex gap-1.5 mb-4 flex-wrap">
            <button onclick="resizeViewport('320px')" class="px-3 py-1 bg-slate-200 hover:bg-slate-300 text-xs font-semibold rounded transition-colors text-slate-700">Phone (xs: 320px)</button>
            <button onclick="resizeViewport('420px')" class="px-3 py-1 bg-slate-200 hover:bg-slate-300 text-xs font-semibold rounded transition-colors text-slate-700">Phone (phone: 420px)</button>
            <button onclick="resizeViewport('800px')" class="px-3 py-1 bg-slate-200 hover:bg-slate-300 text-xs font-semibold rounded transition-colors text-slate-700">Tablet (tablet: 800px)</button>
            <button onclick="resizeViewport('1100px')" class="px-3 py-1 bg-slate-200 hover:bg-slate-300 text-xs font-semibold rounded transition-colors text-slate-700">Laptop (laptop: 1100px)</button>
            <button onclick="resizeViewport('1350px')" class="px-3 py-1 bg-slate-200 hover:bg-slate-300 text-xs font-semibold rounded transition-colors text-slate-700">Desktop (desktop: 1350px)</button>
            <button onclick="resizeViewport('100%')" class="px-3 py-1 bg-slate-300 hover:bg-slate-400 text-xs font-semibold rounded transition-colors text-slate-800 font-bold">Wide (100%)</button>
        </div>

        <!-- Mock Viewport Frame -->
        <div class="border border-slate-300 rounded-2xl bg-white shadow-md overflow-hidden transition-all duration-500 mx-auto" id="viewport-frame" style="width: 100%">
            <!-- Top bar -->
            <div class="bg-slate-100 border-b border-slate-200 px-4 py-2 flex items-center justify-between text-xs text-slate-400 font-mono">
                <div class="flex gap-1">
                    <span class="w-2.5 h-2.5 rounded-full bg-slate-300"></span>
                    <span class="w-2.5 h-2.5 rounded-full bg-slate-300"></span>
                    <span class="w-2.5 h-2.5 rounded-full bg-slate-300"></span>
                </div>
                <div class="bg-slate-200/60 px-3 py-0.5 rounded-md text-slate-600" id="width-indicator">100%</div>
            </div>
            
            <!-- Frame content -->
            <div class="p-6 mock-viewport-container bg-slate-50">
                <div class="responsive-card border p-6 rounded-xl shadow-sm">
                    <div class="flex justify-between items-start mb-4">
                        <h4 class="font-bold text-lg text-slate-800">Адаптивна картка</h4>
                        <span class="px-2.5 py-0.5 rounded text-xs font-bold font-mono">
                            <span class="bp-default-active">default (< 360px)</span>
                            <span class="bp-phone-active">phone (≥ 360px)</span>
                            <span class="bp-tablet-active">tablet (≥ 768px)</span>
                            <span class="bp-laptop-active">laptop (≥ 1024px)</span>
                            <span class="bp-desktop-active">desktop (≥ 1280px)</span>
                            <span class="bp-wide-active">wide (≥ 1600px)</span>
                        </span>
                    </div>
                    <p class="text-sm text-slate-600 mb-4 leading-relaxed">Цей елемент змінює колір фону і рамки в залежності від активованого breakpoint. Клацніть кнопки вгорі для симуляції різної ширини.</p>
                    <div class="flex gap-2 flex-wrap text-[10px] font-bold font-mono">
                        <span class="bp-badge bp-badge-phone px-2 py-1 rounded">phone</span>
                        <span class="bp-badge bp-badge-tablet px-2 py-1 rounded">tablet</span>
                        <span class="bp-badge bp-badge-laptop px-2 py-1 rounded">laptop</span>
                        <span class="bp-badge bp-badge-desktop px-2 py-1 rounded">desktop</span>
                        <span class="bp-badge bp-badge-wide px-2 py-1 rounded">wide</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        function resizeViewport(width) {
            ...
            const widthInd = document.getElementById('width-indicator');
            frame.style.width = width;
            
            // Wait briefly for transition to finish or start, then calculate width
            setTimeout(() => {
                const actualWidth = frame.clientWidth;
                widthInd.innerText = actualWidth + 'px';
            }, 150);
        }
        
        // Initial width check
        setTimeout(() => {
            document.getElementById('width-indicator').innerText = document.getElementById('viewport-frame').clientWidth + 'px';
        }, 300);
        
        window.addEventListener('resize', () => {
            const frame = document.getElementById('viewport-frame');
            if (frame.style.width === '100%' || !frame.style.width) {
                document.getElementById('width-indicator').innerText = frame.clientWidth + 'px';
            }
        });
    </script>
</body>
...
<script src="https://cdn.tailwindcss.com"></script>
<style>
    .rounded-sm-custom { border-radius: 4px; }
    .rounded-md-custom { border-radius: 8px; }
    .rounded-brand-custom { border-radius: 16px; }
    .rounded-xl-custom { border-radius: 24px; }
    .rounded-pill-custom { border-radius: 9999px; }
    .rounded-blob-custom { border-radius: 30% 70% 70% 30% / 30% 30% 70% 70%; }
</style>
</head>
<body class="p-6 bg-slate-50 font-sans">
    <p class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Демонстрація Custom Border Radii</p>
    <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
        <!-- rounded-sm -->
        <div class="bg-violet-600 text-white p-4 flex flex-col justify-center items-center text-center shadow rounded-sm-custom h-28">
            <span class="font-bold text-sm">rounded-sm</span>
            <span class="text-[10px] opacity-80 mt-1">4px</span>
        </div>
        <!-- rounded-md -->
        <div class="bg-violet-600 text-white p-4 flex flex-col justify-center items-center text-center shadow rounded-md-custom h-28">
            <span class="font-bold text-sm">rounded-md</span>
            <span class="text-[10px] opacity-80 mt-1">8px</span>
        </div>
        <!-- rounded-brand -->
        <div class="bg-violet-600 text-white p-4 flex flex-col justify-center items-center text-center shadow rounded-brand-custom h-28">
            <span class="font-bold text-sm">rounded-brand</span>
            <span class="text-[10px] opacity-80 mt-1">16px</span>
        </div>
        <!-- rounded-xl -->
        <div class="bg-violet-600 text-white p-4 flex flex-col justify-center items-center text-center shadow rounded-xl-custom h-28">
            <span class="font-bold text-sm">rounded-xl</span>
            <span class="text-[10px] opacity-80 mt-1">24px</span>
        </div>
        <!-- rounded-pill -->
        <div class="bg-violet-600 text-white p-4 flex flex-col justify-center items-center text-center shadow rounded-pill-custom h-28">
            <span class="font-bold text-sm">rounded-pill</span>
            <span class="text-[10px] opacity-80 mt-1">9999px</span>
        </div>
        <!-- rounded-blob -->
        <div class="bg-violet-600 text-white p-4 flex flex-col justify-center items-center text-center shadow rounded-blob-custom h-28">
            <span class="font-bold text-sm">rounded-blob</span>
            <span class="text-[10px] opacity-85 mt-1 font-mono text-[9px]">30% 70%...</span>
        </div>
    </div>
</body>
</html>
...
<style>
    ...
    .shadow-brand-custom {
        box-shadow: 0 8px 24px oklch(0.6 0.22 270 / 0.4);
    }
</style>
</head>
<body class="p-8 bg-slate-50 font-sans">
    <p class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-6">Демонстрація Custom Box Shadows</p>
    <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
        <!-- shadow-card -->
        <div class="bg-white p-5 rounded-xl border border-slate-100 flex flex-col justify-center items-center text-center shadow-card-custom h-28">
            <span class="font-bold text-slate-700 text-sm">shadow-card</span>
            <span class="text-[10px] text-slate-400 mt-1">Мінімальне піднесення</span>
        </div>
        <!-- shadow-elevated -->
        <div class="bg-white p-5 rounded-xl border border-slate-100 flex flex-col justify-center items-center text-center shadow-elevated-custom h-28">
            <span class="font-bold text-slate-700 text-sm">shadow-elevated</span>
            <span class="text-[10px] text-slate-400 mt-1">Активний стан</span>
        </div>
        <!-- shadow-modal -->
        <div class="bg-white p-5 rounded-xl border border-slate-100 flex flex-col justify-center items-center text-center shadow-modal-custom h-28">
            <span class="font-bold text-slate-700 text-sm">shadow-modal</span>
            <span class="text-[10px] text-slate-400 mt-1">Висока глибина</span>
        </div>
        <!-- shadow-brand -->
        <div class="bg-white p-5 rounded-xl border border-slate-100 flex flex-col justify-center items-center text-center shadow-brand-custom h-28">
            <span class="font-bold text-slate-700 text-sm text-violet-600">shadow-brand</span>
            <span class="text-[10px] text-violet-400 mt-1">Кольорове свічення</span>
        </div>
    </div>
</body>
</html>
...
<style>
    :root {
        --color-brand-standard: oklch(0.6 0.2 270);
    }
    
    /* Варіант A: Використовує CSS-змінну */
    .bg-brand-standard {
        background-color: var(--color-brand-standard);
        transition: background-color 0.5s;
    }
    
    /* Варіант B: Закомпільовано намертво (inline) */
    .bg-brand-inline {
        background-color: oklch(0.6 0.2 270);
        transition: background-color 0.5s;
    }
</style>
</head>
<body class="p-6 bg-slate-50 font-sans flex flex-col items-center">
    <div class="w-full max-w-md bg-white p-6 rounded-2xl shadow-md border border-slate-100">
        <p class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Демонстрація: @theme vs @theme inline</p>
        
        <div class="grid grid-cols-2 gap-4 mb-6">
            <!-- Box A -->
            <div class="bg-brand-standard text-white p-4 rounded-xl shadow-sm text-center flex flex-col justify-center h-28">
                <span class="font-bold text-sm">Варіант A (@theme)</span>
                <span class="text-[9px] opacity-80 mt-1 font-mono">var(--color-brand-standard)</span>
            </div>
            
            <!-- Box B -->
            <div class="bg-brand-inline text-white p-4 rounded-xl shadow-sm text-center flex flex-col justify-center h-28">
                <span class="font-bold text-sm">Варіант B (inline)</span>
                <span class="text-[9px] opacity-80 mt-1 font-mono">oklch(0.6 0.2 270)</span>
            </div>
        </div>

        <button onclick="toggleCSSVariable()" class="w-full py-2.5 bg-violet-600 text-white font-semibold rounded-xl text-sm shadow hover:bg-violet-700 transition-colors">
            Змінити значення CSS змінної у :root
        </button>
    </div>

    <script>
        let isPurple = true;
        function toggleCSSVariable() {
            const root = document.documentElement;
            if (isPurple) {
                root.style.setProperty('--color-brand-standard', 'oklch(0.7 0.18 145)'); // Зелений
                isPurple = false;
            } else {
                root.style.setProperty('--color-brand-standard', 'oklch(0.6 0.2 270)'); // Фіолетовий
                isPurple = true;
            }
        }
    </script>
...
<style>
    ...
    .base-preview-container a {
        color: oklch(0.6 0.22 270);
        text-decoration: underline;
        text-underline-offset: 3px;
    }
    /* Стилізація виділення тексту */
    .base-preview-container ::selection {
        background: oklch(0.6 0.22 270 / 0.2);
    }
</style>
</head>
<body class="p-6 bg-slate-50">
    <p class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Демонстрація глобальних стилів @layer base</p>
    <div class="base-preview-container p-6 bg-white rounded-2xl shadow-sm border border-slate-100 max-w-md">
        <h1>Стилізований Заголовок h1</h1>
        <p class="text-sm mb-4">Цей абзац демонструє базові налаштування шрифту та кольору. Ви можете <span class="font-semibold text-violet-600">виділити мишкою цей текст</span>, щоб побачити кастомний колір виділення (selection).</p>
        <p class="text-sm">Читати більше про <a href="#" onclick="event.preventDefault()">дизайн-системи</a> на нашому сайті.</p>
    </div>
</body>
</html>
```
...
<style>
    /* Імітуємо CSS компоненти з файлу */
    .btn {
        display: inline-flex;
        align-items: center;
...
<body class="p-6 bg-slate-50 font-sans">
    <div class="flex flex-col gap-4 max-w-md mx-auto">
        <p class="text-xs font-bold text-slate-400 uppercase tracking-widest">Демонстрація @layer components та перевизначення утилітами</p>
        
        <!-- Interactive Card component -->
        <div id="demo-card" class="card">
            <h4 class="font-bold text-slate-900 mb-2">Форма входу (.card)</h4>
            <div class="space-y-3 mb-4">
                <input type="email" placeholder="Email (.input)" class="input">
                <input type="password" placeholder="Password (.input)" class="input">
            </div>
            <div class="flex gap-2 justify-end">
                <button class="btn btn-ghost">Cancel</button>
                <button id="primary-btn" class="btn btn-primary">Login</button>
            </div>
        </div>
        
        <!-- Controls -->
        <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between text-xs">
            <span class="text-slate-500 font-medium">Перемогти компоненти утилітами:</span>
            <button onclick="toggleOverrides()" class="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 border border-slate-300 font-semibold rounded text-slate-700 transition-colors">
                Застосувати утиліти
            </button>
        </div>
    </div>
    ...
    <script>
        ...
        function toggleOverrides() {
            ...
            if (!overridden) {
                // Додаємо прямі utility класи, які за специфічністю вищі за компоненти
                card.classList.add('rounded-none', 'bg-red-50', 'border-red-300');
                btn.classList.add('rounded-none', 'bg-red-600', 'hover:bg-red-700');
                overridden = true;
            } else {
                card.classList.remove('rounded-none', 'bg-red-50', 'border-red-300');
                btn.classList.remove('rounded-none', 'bg-red-600', 'hover:bg-red-700');
                overridden = false;
            }
        }
    </script>
...
<style>
    ...
    .dark .variant-demo-card {
        background-color: #0f172a;
        color: #ffffff;
        border-color: #1e293b;
    }
    
    /* ocean:bg-cyan-950 ocean:text-cyan-100 */
    [data-theme="ocean"] .variant-demo-card {
        background-color: #083344; /* cyan-950 */
        color: #cffafe; /* cyan-100 */
        border-color: #155e75;
    }
    
    /* compact:p-2 compact:text-xs */
    .compact .variant-demo-card {
        padding: 0.5rem;
        font-size: 0.75rem;
    }
    .compact .variant-demo-card h4 {
        font-size: 0.875rem;
        margin-bottom: 0.25rem;
    }
    
    /* rtl:text-right */
    [dir="rtl"] .variant-demo-card {
        text-align: right;
    }
</style>
</head>
<body class="p-6 bg-slate-50 font-sans flex flex-col items-center">
    <div class="w-full max-w-md" id="parent-container">
        <p class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Демонстрація Custom Variants</p>
        
        <!-- Target Card -->
        <div class="variant-demo-card shadow-md mb-6">
            <h4 class="font-bold text-lg mb-2">Наш Компактний Компонент</h4>
            <p class="text-sm opacity-80 leading-relaxed">Цей текст реагує на активовані модифікатори (варіанти), змінюючи колір, розмір, напрямок або тему.</p>
        </div>
        
        <!-- Toggles -->
        <div class="grid grid-cols-2 gap-2 text-xs">
            <button onclick="toggleVar('dark')" class="px-3 py-2 bg-white hover:bg-slate-100 border border-slate-200 font-semibold rounded-lg shadow-sm text-slate-700 transition-colors">
                Toggle Dark Mode (.dark)
            </button>
            <button onclick="toggleVar('ocean')" class="px-3 py-2 bg-white hover:bg-slate-100 border border-slate-200 font-semibold rounded-lg shadow-sm text-slate-700 transition-colors">
                Toggle Ocean Theme ([data-theme])
            </button>
            <button onclick="toggleVar('compact')" class="px-3 py-2 bg-white hover:bg-slate-100 border border-slate-200 font-semibold rounded-lg shadow-sm text-slate-700 transition-colors">
                Toggle Compact Mode (.compact)
            </button>
            <button onclick="toggleVar('rtl')" class="px-3 py-2 bg-white hover:bg-slate-100 border border-slate-200 font-semibold rounded-lg shadow-sm text-slate-700 transition-colors">
                Toggle RTL (dir="rtl")
            </button>
        </div>
    </div>
    ...
    <script>
        function toggleVar(mode) {
            ...
            if (mode === 'dark') {
                ...
                if (container.getAttribute('data-theme') === 'ocean') {
                ...
                if (container.getAttribute('dir') === 'rtl') {
