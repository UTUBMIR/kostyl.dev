...
```mermaid
stateDiagram-v2
    [*] --> Clean : завантажено з БД<br/>(findById)
    [*] --> New : створено у пам'яті<br/>(new Author(...))
...
```mermaid
graph TD
    CL["Клієнтський код<br/>(Service / Handler)"]
    SS["Session<br/>(координатор)"]

    subgraph SL ["Session Layer"]
        IM["Identity Map<br/>findAuthor → cache hit/miss"]
        UW["Unit of Work<br/>newEntities / dirtyEntities<br/>deletedEntities"]
        CT["Change Tracker<br/>snapshots (optional)"]
    end

    subgraph RL ["Repository Layer"]
        AR["JdbcAuthorRepository<br/>(SQL: INSERT, UPDATE, SELECT, DELETE)"]
