...
classDiagram
    ...
    List~T~ <|.. ArrayList~T~
    List~T~ <|.. LazyList~T~
    Author o-- LazyList~T~ : audiobooks field\n(клієнт бачить List)

    note for LazyList~T~ "ensureLoaded() виконує SQL\nтільки при першому зверненні"
```
...
graph LR
    ...
    subgraph Repo ["Репозиторій шар"]
        JAR["JdbcAuthorRepository\nmapRow() → initLazyAudiobooks()"]
        JBR["JdbcAudiobookRepository\nfindByAuthorId()"]
    end
