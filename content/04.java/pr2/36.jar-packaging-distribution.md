...
> [!IMPORTANT]
> `jpackage` вимагає **виключно WiX Toolset версії 3.x** (версії 4.x та 5.x не підтримуються утилітою автоматично). Тому необхідно встановлювати саме версію 3.

Для встановлення WiX Toolset v3 скористайтеся одним із пакетних менеджерів для Windows:

::tabs

::tabs-item{label="Scoop" icon="i-simple-icons-scoop"}
...
::tabs-item{label="Chocolatey" icon="i-simple-icons-chocolatey"}
...
::tabs-item{label="Winget" icon="i-simple-icons-windows"}
...
> [!WARNING]
> **Проблема з кирилицею у метаданих на Windows**
> WiX Toolset v3 за замовчуванням використовує західноєвропейську кодову сторінку (наприклад, `1252`). Якщо значення параметрів `--description`, `--vendor` чи `--name` містять кириличні символи, WiX не зможе скомпілювати інсталятор і поверне помилку виду:
> `light.exe : error LGHT0311 : A string was provided with characters that are not available in the specified database code page '1252'.`
> 
  ...
  build-installers:
    ...
    steps:
      ...
      - name: Build Fat JAR
        ...
        working-directory: helloworldjavafx

      # 6. Підготовка директорії вхідних файлів для jpackage
      - name: Prepare jpackage input
        run: |
          mkdir -p helloworldjavafx/jpackage-input
          cp helloworldjavafx/target/helloworldjavafx-fat.jar helloworldjavafx/jpackage-input/
      ...
      - name: Package installer (Windows)
        ...
        run: |
          jpackage `
            ...
            --main-jar helloworldjavafx-fat.jar `
            --main-class com.arakviel.Launcher `
            --icon packaging/windows/audiobook.ico `
            --dest dist/ `
            --win-dir-chooser `
            --win-menu `
