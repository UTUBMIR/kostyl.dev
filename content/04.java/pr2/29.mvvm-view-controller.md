...
Створимо файл `AudiobookListView.fxml` для екрану зі списком аудіокниг:
...
**Завдання 1.1:** Створіть FXML-файл `AuthorListView.fxml` з таблицею авторів (колонки: First Name, Last Name, Birth Year) та кнопками Add, Edit, Delete.
...
**Завдання 2.1:** Додайте у `AudiobookListView.fxml` CheckBox "Show only favorites". Прив'яжіть його до `BooleanProperty showOnlyFavorites` у ViewModel через Bidirectional Binding.
...
**Завдання 3.2:** Модифікуйте `AudiobookApp`, щоб він завантажував `AuthorListView.fxml` через `FXMLLoader` з `ControllerFactory`.
