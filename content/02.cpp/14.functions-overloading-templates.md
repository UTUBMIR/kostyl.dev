...
```mermaid
flowchart TD
    A["Виклик: foo(arg)"] --> B{"Точний збіг<br/>(exact match)?"}
    B -- "так" --> G["✅ Викликає цю функцію"]
    B -- "ні" --> C{"Збіг після trivial<br/>conversion (const, &)?"}
    C -- "так" --> G
    C -- "ні" --> D{"Збіг після<br/>розширення типу?<br/>(char→int, float→double)"}
    D -- "так" --> G
    D -- "ні" --> E{"Збіг після<br/>неявного перетворення?"}
    E -- "так" --> F{"Збіг лише один?<br/>Чи кілька однаково підходять?"}
    F -- "один" --> G
    F -- "кілька" --> H["❌ Помилка:<br/>ambiguous call"]
    E -- "ні" --> I["❌ Помилка:<br/>no matching function"]
