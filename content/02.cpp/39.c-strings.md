...
int main()
{
    ...
    std::cout << "Різниця       = " << sizeof(name) - std::strlen(name) << "\n"; // 1
...
```cpp [Declaration1.cpp] showLineNumbers
...
```cpp [Declaration2.cpp] showLineNumbers
char name[10] = "John";
...
```cpp [Declaration3.cpp] showLineNumbers
...
```cpp [OutputDemo.cpp] showLineNumbers
#include <iostream>
...
::terminal-preview{title="./OutputDemo"}
...
```cpp [CinInput.cpp] showLineNumbers
#include <iostream>
...
int main()
{
    ...
    std::cin >> name;  // ⚠️ Зупиняється на пробілі, переповнення можливе!
...
```cpp [SafeInput.cpp] showLineNumbers
#include <iostream>
...
int main()
{
    ...
    std::cin.getline(fullName, sizeof(fullName)); // Безпечно!
    // Аргументи: (буфер, максимальний розмір включно з '\0')
    // Читає до '\n' або до (sizeof - 1) символів — завжди додає '\0'

    std::cout << "Привіт, " << fullName << "!\n";
    std::cout << "Довжина: " << std::strlen(fullName) << " символів\n";

    return 0;
}
```

::terminal-preview{title="./SafeInput"}
