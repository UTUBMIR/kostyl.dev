...
```mermaid
flowchart LR
    A["Виклик:<br/>applyDiscount(250.0, 0.1)"] --> B["Функція отримує:<br/>price=250.0<br/>discountRate=0.1"]
    B --> C["Обчислення:<br/>discountAmount = 25.0<br/>finalPrice = 225.0"]
    C --> D["return 225.0"]
    D --> E["Результат у викликачі:<br/>total = 225.0"]
...
```mermaid
flowchart LR
    A["main:<br/>value = 10"] -- "tryToModify(value)<br/>копіюється значення 10" --> B["tryToModify:<br/>number = 10<br/>(окрема копія)"]
    B -- "number = 999<br/>(повертається)" --> A
