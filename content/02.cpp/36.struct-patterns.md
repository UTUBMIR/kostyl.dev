...
Оголосіть `struct DivisionResult { bool ok; double value; std::string error; };`. Напишіть `DivisionResult safeDivide(double a, double b)` — повертає помилку при `b == 0`, інакше `a / b`. Напишіть кілька тест-кейсів: `10/2`, `7/0`, `-15/3`.
::
